	...
	public:
		...
		virtual ~ScopedTimer() noexcept {
			if (m_printOnExit) {
				std::cout <<
				"Elapsed: " <<
				std::chrono::duration_cast<_duration>(std::chrono::system_clock::now() - m_start).count() <<
				" " << chronoDurationPrettyName<_duration>() << std::endl;
			}
		}

		size_t getCurDuration() const noexcept {
			return std::chrono::duration_cast<_duration>(std::chrono::system_clock::now() - m_start).count();
		}
